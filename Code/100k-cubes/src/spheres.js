...
varying vec3 vColor;
  ...
  void main(){
    ...
    float aProgress = mod(aCurve.y, 1.); // Stop. Spinning. FFS. 🤦‍♀️ 
...
export class Spheres extends THREE.Mesh {
  ...
  init() {
    let instancedGeometry = new THREE.InstancedBufferGeometry().copy(this.config.useCube ? baseCube : baseGeometry);
    ...
    instancedGeometry.addAttribute('aCurve', new THREE.InstancedBufferAttribute(new Float32Array(aCurve), 4, false));
    instancedGeometry.addAttribute('aColor', new THREE.InstancedBufferAttribute(new Float32Array(aColor), 3, false));
